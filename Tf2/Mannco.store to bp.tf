...
// Function to get the effect ID from the effects list
...
function extractItemInfo() {
    var itemInfo = document.querySelector(".item-info__container");
    var itemNameElement = itemInfo.querySelector(".item-info__name");
    var itemName = itemNameElement ? itemNameElement.innerText.trim() : "";
    var qualityElement = itemInfo.querySelector("dd");
    var quality = qualityElement ? qualityElement.innerText.trim() : "";
    return { itemName, quality };
}

// Function to open Backpack.tf stats in a new tab for every item except unusuals
function openBackpackStats(itemName, quality) {
    // Remove "The" or "the" from the item name if it exists
    itemName = itemName.replace(/^the /i, "");

    // Remove "Uncraftable" from the item name if it exists
    itemName = itemName.replace(/uncraftable/i, "").trim();

    // Extract the quality from the item name
    var qualityMatch = itemName.match(/(\w+ Collector's|\bStrange\b|\bVintage\b|\bGenuine\b|\bUnique\b|\bHaunted\b|\bSelf-Made\b)/i);
    if (qualityMatch) {
        quality = qualityMatch[0];
        // Remove the matched quality from the item name
        itemName = itemName.replace(qualityMatch[0], "").trim();
    }

    var craftableElement = [...document.querySelectorAll(".item-info__list dt")].find(dt => dt.textContent.trim() === "Craftable");
    var craftable = craftableElement ? craftableElement.nextElementSibling.textContent.trim() : "";
    craftable = craftable.toLowerCase().includes("yes") ? "Craftable" : "Non-Craftable";

    // Extracting the effect name from the item name
    var effectMatch = itemName.match(/★\s*(.*?)\s*Unusual/i);
    var effect = effectMatch ? effectMatch[1] : null;

    if (effect) {
        // If it's an unusual item, use the second script logic
        openBackpackStatsForUnusual(itemName, quality);
    } else {
        // For all other items, use the first script logic
        var url = `https://backpack.tf/stats/${quality}/${itemName}/Tradable/${craftable}`;
        window.open(url, "_blank"); // Open in new tab
    }
}

// Function to open Backpack.tf stats in a new tab for unusual items
function openBackpackStatsForUnusual(itemName, quality) {
    // Remove "The" or "the" from the item name if it exists
    itemName = itemName.replace(/^the /i, "").replace(/^the /i, "");
    ...
    if (effect) {
        // Remove the effect name and "Unusual" tag from the item name
        itemName = itemName.replace(effectMatch[0], "").replace("Unusual", "").trim();
        var effectID = getEffectID(effect);
        var url = `https://backpack.tf/stats/${quality}/${itemName}/Tradable/${craftable}/${effectID}`;
        window.open(url, "_blank"); // Open in new tab
    }
}

// Function to create and style the info box
...
// Function to append the info box to the body
function appendInfoBox(box) {
    var btnStats = document.createElement("button");
    btnStats.id = "btnStats";
    btnStats.textContent = "Backpack.tf Stats";
    btnStats.style.padding = "5px 10px";
    btnStats.style.borderRadius = "5px";
    btnStats.style.backgroundColor = "#007bff";
    btnStats.style.color = "#fff";
    btnStats.onclick = function() {
        var { itemName, quality } = extractItemInfo();
        openBackpackStats(itemName, quality);
    };
    box.appendChild(btnStats);
